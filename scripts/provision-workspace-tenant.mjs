...
//     --controller <oidc-subject> --security <oidc-subject> [--preparer <oidc-subject>] [--clerk <oidc-subject>] [--billing <oidc-subject>] [--treasury <oidc-subject>] [--auditor <oidc-subject>] [--issuer <url>]
...
loadLocalEnv();
const {values}=parseArgs({options:{slug:{type:'string'},name:{type:'string'},mode:{type:'string',default:'demo'},controller:{type:'string'},security:{type:'string'},preparer:{type:'string'},clerk:{type:'string'},billing:{type:'string'},treasury:{type:'string'},auditor:{type:'string'},issuer:{type:'string'},json:{type:'boolean',default:false}}});
...
try{
 ...
 const result=await inTransaction(db,{tenantId,principalId:null,traceId:'provision-'+tenantId},async tx=>{
  ...
  const people={};
  for(const role of ['controller','security','preparer','clerk','billing','treasury','auditor'])if(values[role])people[role]=await identity.resolvePrincipal(tx,tenantId,{issuer,subject:values[role],displayName:role});
  ...
  const assign={controller:'controller',security:'security_admin',preparer:'accountant',clerk:'clerk',billing:'billing',treasury:'treasury',auditor:'auditor'};
  const memberships=[];
