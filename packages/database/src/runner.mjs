...
const legacy=JSON.parse(readFileSync(new URL('../legacy-checksums.json',import.meta.url),'utf8'));
...
export async function applyMigrations(db, files, { metadataSchema = 'public', transaction = true } = {}) {
  ...
  try {
    ...
    for(const {version,sql} of files) {
      ...
      const legacyWindowsHash=createHash('sha256').update(canonical.replace(/\n/g,'\r\n')).digest('hex');
      const old=await db.query('select sha256 from '+table+' where version=$1',[version]);
      if(old.rowCount && old.rows[0].sha256!==hash && old.rows[0].sha256!==legacyWindowsHash && !(legacy[version]?.recorded===old.rows[0].sha256 && legacy[version]?.canonical===hash)) throw new Error('Migration checksum mismatch: '+version);
