...
export async function postDocument(tx,ctx,entityId,id,input,expectedVersion,{commandId=null}={}){
 ...
 if(row.kind==='bill'){
  if(payable>0n)await tx.query("insert into lara.open_items(tenant_id,entity_id,document_id,side,party_id,original_amount,currency,due_date) values($1,$2,$3,'AP',$4,$5,$6,$7)",[ctx.tenantId,entityId,id,row.party_id,decimal(payable,6),row.currency,row.due_schedule[0]?.dueDate||iso(row.document_date)]);
...
export async function settlePayment(tx,ctx,entityId,id,input,expectedVersion,{commandId=null}={}){
 ...
 const lines=[{accountId:advance?profile.advanceAccountId:profile.apAccountId,branchId:branch.id,dimensions:{},debit:decimal(gross,6),credit:'0'},{accountId:profile.cashAccountId,branchId:branch.id,dimensions:{},debit:'0',credit:decimal(cash,6)}];
 ...
 const entryId=(await tx.query('select lara.post_journal_entry($1::jsonb) as id',[JSON.stringify({tenantId:ctx.tenantId,entityId,bookId:settlementRow.book_id,sourceType:'settlement',sourceId:settlementRow.id,sourceVersion:Number(settlementRow.content_version),purpose:'posting',accountingDate:input.valueDate,documentDate:input.valueDate,description:(advance?'Advance to ':'Payment to ')+party.legal_name+' '+input.externalReference.trim(),currency:settlementRow.currency,manual:false,postingActor:ctx.principalId,commandId,lines})])).rows[0].id;
 ...
 return presult(updated,{journalEntryIds:[entryId]});
}
