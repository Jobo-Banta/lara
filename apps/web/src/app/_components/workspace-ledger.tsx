...
export function Capabilities({entityId,me,can,tick,refresh}:Ctx){
 ...
 const caps:[string,string][]=[['workspace','Finance workspace (P02)'],['general_ledger','General ledger and close (P03)'],['sales','Sales invoicing and receivables (P04)'],['purchasing','Purchasing payables and expenses (P05)'],['treasury','Treasury cash and bank reconciliation (P06)'],['compliance','Tax compliance and e-invoicing (P07)'],['fi_coexistence','Financial institution coexistence (P08)'],['multi_currency','Multiple currencies and separate books (P09)'],['inventory','Inventory costing and three-way matching (P10)'],['assets','Assets, recurring work and recognition schedules (P11)'],['ai_assistance','Evidence-backed AI assistance (P12)'],['portals','Customer and supplier portals and messaging (P13)'],['firm_workspace','Accounting firm multi-client workspace (P14)'],['group_accounting','Intercompany and consolidation (P15)'],['planning','Budgets, cost allocation and project accounting (P16)'],['leases','Lease billing (P17A)'],['statutory_discounts','Statutory discounts (P17B)'],['marketplace_pos','Marketplace and POS integration (P17C)'],['payroll_data','Payroll data and remittances (P17D)'],['local_obligations','Local obligations (P17E)'],['report_authoring','Report and custom-field authoring (P18A)'],['rule_proposals','Rule impact and tax proposals (P18B)'],['client_tools','Client draft tools (P18C)'],['industry_packs','Industry pack lifecycle (P18D)']];
 return <>{caps.map(([code,label])=><section key={code} className="demo-card"><h2>{label}</h2><p>{rows?.includes(code)?'Active on this entity.':'Not active. Request activation with the evidence the profile requires; a different principal approves.'}</p>{!rows?.includes(code)&&can('capability.activate')&&<Editor id={'cap'+code+entityId} label="Request or approve activation" onSave={async d=>!!await cmd.run('POST','/capabilities/activate',{entityId,body:{capability:code,profileVersion:d.profileVersion,evidenceIds:[d.evidenceId],reason:d.reason}})}>{input('Profile version','profileVersion',code==='workspace'?'p02.1':code==='sales'?'p04.1':code==='purchasing'?'p05.1':code==='treasury'?'p06.1':code==='compliance'?'p07.1':code==='fi_coexistence'?'p08.1':code==='multi_currency'?'p09.1':code==='inventory'?'p10.1':code==='assets'?'p11.1':code==='ai_assistance'?'p12.1':code==='portals'?'p13.1':'p03.1','text',{required:true})}<label>Activation evidence<select name="evidenceId" required><option value="">Select available evidence</option>{(evidence.items||[]).map(e=><option key={e.id} value={e.id}>{e.filename}</option>)}</select></label>{input('Reason','reason','','text',{required:true})}</Editor>}</section>)}<ErrorPanel error={cmd.error} onRetry={cmd.retry} onReload={cmd.reload} canRetry={cmd.canRetry}/></>;
}
