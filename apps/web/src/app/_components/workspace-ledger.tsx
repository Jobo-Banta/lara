...
export function LedgerImports({entityId,me,can,tick,refresh}:Ctx){
 ...
 return <>{can('import.create')&&<><h2>New opening import</h2><p>Upload the CSV as evidence first (columns: source_key, account_code, branch_code, accounting_date, debit, credit). Validation checks every row against the chart; approval must come from a different principal; commit posts one opening entry.</p><Editor id={'import-new'+entityId} label="Stage import" resetOnSave onSave={async d=>!!await cmd.run('POST','/imports',{entityId,body:{kind:'openings',evidenceId:d.evidenceId,mappingVersion:d.mappingVersion,sourceId:d.sourceId,externalBatchId:d.externalBatchId,cutoffDate:d.cutoffDate}})}><label>CSV evidence<select name="evidenceId" required><option value="">Select available CSV</option>{(evidence.items||[]).filter(e=>e.mime==='text/csv').map(e=><option key={e.id} value={e.id}>{e.filename}</option>)}</select></label>{input('Mapping version','mappingVersion','v1','text',{required:true})}{input('Source system','sourceId','legacy-gl','text',{required:true})}{input('External batch id','externalBatchId','','text',{required:true})}{input('Cutoff date','cutoffDate','','date',{required:true})}</Editor></>}
  <ErrorPanel error={cmd.error||list.error} onRetry={cmd.retry} onReload={cmd.reload} canRetry={cmd.canRetry}/>
