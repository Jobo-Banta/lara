...
type ApiError={code:string,message:string,traceId?:string,fieldErrors:{path:string,message:string}[],retryable:boolean,resourceVersion?:number};
const nav:[string,string][]=[['My work','/work'],['Overview','/overview'],['Parties','/parties'],['Evidence','/evidence'],['Obligations','/obligations'],['Money in','/sales/invoices'],['Money out','/purchases/bills'],['Close','/close/2026-09'],['Compliance','/compliance'],['Reports','/reports']];
const roadmap:Record<string,string>={'/sales/invoices':'Sales invoicing arrives with the ledger and sales modules (P03–P04).','/purchases/bills':'Purchasing and payables arrive with P05.','/close/2026-09':'Period close arrives with the general ledger (P03).','/compliance':'Compliance and e-invoicing arrive with P07.','/reports':'Financial reports arrive with the general ledger (P03).','/payments':'Payments arrive with treasury (P06).','/bank/reconcile':'Bank reconciliation arrives with treasury (P06).'};
const uuid=()=>crypto.randomUUID();
const when=(v?:string)=>v?new Date(v).toLocaleString('en-PH',{timeZone:'Asia/Manila',dateStyle:'medium',timeStyle:'short'}):'—';
const input=(label:string,name:string,value='',type='text',extra:Row={})=><label key={name}>{label}<input name={name} defaultValue={value} type={type} {...extra}/></label>;
const select=(label:string,name:string,options:[string,string][],value='')=><label key={name}>{label}<select name={name} defaultValue={value}>{options.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label>;

class RequestError extends Error{status:number;body:ApiError;constructor(status:number,body:ApiError){super(body.message);this.status=status;this.body=body;}}
async function api(method:string,path:string,{body,entityId,ifMatch,key,raw,headers={}}:{body?:unknown,entityId?:string|null,ifMatch?:number|string,key?:string,raw?:BodyInit,headers?:Record<string,string>}={}){
...
const asError=(e:unknown):ApiError=>e instanceof RequestError?e.body:{code:'DEPENDENCY_UNAVAILABLE',message:'Connection interrupted. Retry when available.',fieldErrors:[],retryable:true};
...
function useCommand(onDone:()=>Promise<unknown>|unknown){
...
function ErrorPanel({error,onRetry,onReload,canRetry}:{error:ApiError|null,onRetry?:()=>void,onReload?:()=>void,canRetry?:boolean}){
...
function Editor({id,children,onSave,label='Save',disabled=false}:{id:string,children:ReactNode,onSave:(data:Row)=>Promise<boolean>,label?:string,disabled?:boolean}){
 ...
 async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();setSaving(true);try{if(await onSave(Object.fromEntries(new FormData(e.currentTarget)))){try{sessionStorage.removeItem('lara-ws-draft:'+id);}catch{}setDirty(false);}}finally{setSaving(false);}}
...
const table=(head:string[],rows:ReactNode[][],empty:string)=><div className="table-scroll" tabIndex={0} role="region" aria-label={head.join(', ')+' table'}><table><thead><tr>{head.map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{rows.map((r,i)=><tr key={i}>{r.map((c,j)=><td key={j}>{c}</td>)}</tr>)}</tbody></table>{!rows.length&&<p>{empty}</p>}</div>;
