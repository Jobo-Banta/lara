...
import {AssetRegister,AssetSchedules} from './workspace-assets';
import {Assistant} from './workspace-assistant';
import {PortalAdmin,PortalHome} from './workspace-portal';
import {FirmWorkspace,ClientMandates} from './workspace-firm';
import {GroupSetup,IntercompanyPairs,Consolidations} from './workspace-group';
import {Budgets,Allocations,Projects} from './workspace-planning';
import {Leases,Discounts,Channels,Payroll,LocalObligations} from './workspace-localops';
import {Reports,RuleProposals,ToolGrants,Packs} from './workspace-extend';

const nav:[string,string][]=[['My work','/work'],['Overview','/overview'],['Parties','/parties'],['Evidence','/evidence'],['Obligations','/obligations'],['Journals','/ledger/journals'],['Chart of accounts','/ledger/accounts'],['Periods and close','/ledger/periods'],['Imports','/ledger/imports'],['Reports','/reports'],['Invoices','/sales/invoices'],['Orders and quotes','/sales/orders'],['Receipts','/sales/collections'],['Customers','/sales/customers'],['Bills','/purchases/bills'],['Purchase orders','/purchases/orders'],['Expense claims','/purchases/claims'],['Payments','/payments'],['Suppliers','/purchases/suppliers'],['Bank accounts','/bank/accounts'],['Reconcile','/bank/reconcile'],['Transfers','/bank/transfers'],['Checks','/bank/checks'],['Cash','/bank/cash'],['Compliance','/compliance'],['Source ownership','/institution/ownership'],['Feeds','/institution/feeds'],['Branches','/institution/branches'],['Institution tax','/institution/tax'],['Books','/books'],['FX rates','/fx/rates'],['Revaluations','/fx/revaluations'],['Inventory','/inventory'],['Stock movements','/inventory/movements'],['Counts','/inventory/counts'],['Landed costs','/inventory/landed-costs'],['Assets','/assets'],['Schedules','/assets/schedules'],['Assistant','/assistant'],['Portal','/portal'],['Firm','/firm'],['Mandates','/firm/mandates'],['Group','/group'],['Intercompany','/group/pairs'],['Consolidations','/group/consolidations'],['Budgets','/planning'],['Allocations','/planning/allocations'],['Projects','/planning/projects'],['Leases','/local'],['Discounts','/local/discounts'],['Channels','/local/channels'],['Payroll','/local/payroll'],['Local obligations','/local/obligations'],['Reports','/extend'],['Rule proposals','/extend/rules'],['Client tools','/extend/tools'],['Packs','/extend/packs']];
// Every planned module has shipped a release candidate; the roadmap placeholder map stays for later increments.
const roadmap:Record<string,string>={};
// The explicit client context a firm delegate is working in (P14); leaving it returns to the firm.
function ClientContext(){const [label,setLabel]=useState<string|null>(null);useEffect(()=>{try{setLabel(sessionStorage.getItem('lara-tenant')?sessionStorage.getItem('lara-client-label')||'client':null);}catch{}},[]);if(!label)return null;return <p role="status" className="client-context">Client context: <strong>{label}</strong> <button type="button" className="link" onClick={()=>{try{sessionStorage.removeItem('lara-tenant');sessionStorage.removeItem('lara-entity');sessionStorage.removeItem('lara-client-label');}catch{}window.location.href='/firm';}}>Leave client</button></p>;}
export default function Workspace(){
 ...
 // External identities (P13): a portal member holds the portal permissions only; every internal template holds task.read.
 const portalMember=!!me&&!me.permissions.includes('task.read')&&me.permissions.includes('session.read')&&me.entityIds.length>0;
 ...
 else if(portalMember){title='Portal';content=<PortalHome entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 ...
 else if(path==='/firm'){title='Firm';content=<FirmWorkspace entityId={entityId} me={me} can={can} tick={tick} refresh={refresh}/>;}
 ...
 else if(path==='/assets'){title='Assets';content=<AssetRegister entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/assets/schedules'){title='Schedules';content=<AssetSchedules entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/assistant'){title='Assistant';content=<Assistant entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/portal'){title='Portal';content=<PortalAdmin entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/firm/mandates'){title='Firm mandates';content=<ClientMandates entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/group'){title='Group';content=<GroupSetup entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/group/pairs'){title='Intercompany pairs';content=<IntercompanyPairs entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/group/consolidations'){title='Consolidations';content=<Consolidations entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/planning'){title='Budgets';content=<Budgets entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/planning/allocations'){title='Allocations';content=<Allocations entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/planning/projects'){title='Projects';content=<Projects entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/local'){title='Leases';content=<Leases entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/local/discounts'){title='Statutory discounts';content=<Discounts entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/local/channels'){title='Channels and POS';content=<Channels entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/local/payroll'){title='Payroll data';content=<Payroll entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/local/obligations'){title='Local obligations';content=<LocalObligations entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/extend'){title='Reports';content=<Reports entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/extend/rules'){title='Rule proposals';content=<RuleProposals entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/extend/tools'){title='Client tools';content=<ToolGrants entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
 else if(path==='/extend/packs'){title='Industry packs';content=<Packs entityId={entityId!} me={me} can={can} tick={tick} refresh={refresh}/>;}
