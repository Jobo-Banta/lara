...
// ---------------------------------------------------------------------------
function DocumentEditor({entityId,bookId,kind,customers,accounts,rules,branches,initial,onSave,label}:{entityId:string,bookId:string,kind:string,customers:Row[],accounts:Row[],rules:Row[],branches:Row[],initial?:Row,onSave:(body:Row)=>Promise<boolean>,label:string}){
 ...
 return <Editor id={'doc'+(initial?.id||'new')+kind+entityId+(initial?.version||0)} label={label} onSave={async d=>onSave({kind,branchId:d.branchId,bookId,partyId:d.partyId,documentDate:d.documentDate,accountingDate:d.accountingDate,currency:'PHP',ruleProfileVersion:d.ruleProfileVersion,...(d.externalReference?{externalReference:d.externalReference}:{}),...(initial?.sourceDocumentId?{sourceDocumentId:initial.sourceDocumentId}:{}),lines:lines.map(l=>({description:l.description,quantity:l.quantity,unitPrice:l.unitPrice,discount:l.discount||'0',priceBasis:l.priceBasis,accountId:l.accountId,...(l.taxCodeId?{taxCodeId:l.taxCodeId}:{}),dimensions:{}})),evidenceIds:[]})}>
  ...
  {input('Tax profile version','ruleProfileVersion',initial?.ruleProfileVersion||'ph-vat-2026','text',{required:true})}{input('Customer reference (optional)','externalReference',initial?.externalReference||'','text',{maxLength:500})}
...
export function SalesInvoices({entityId,me,can,tick,refresh}:Ctx){
 ...
 return <>{can('invoice.prepare')&&bookId&&<><h2>New invoice</h2><DocumentEditor entityId={entityId} bookId={bookId} kind="invoice" {...refs} label="Save draft" onSave={async body=>{const r=await cmd.run('POST','/invoices',{entityId,body});if(r)window.location.assign('/sales/invoices/'+r.data.id);return !!r;}}/></>}
...
export function SalesInvoiceDetail({entityId,id,me,can,tick,refresh}:Ctx&{id:string}){
 ...
 return <>
  <section className="demo-card"><h2>{kindLabel[d.kind]} {d.officialNumber||'(draft)'}</h2><p>{customer?.legalName||'Customer'} · {d.documentDate} · accounting {d.accountingDate}{d.externalReference?' · '+d.externalReference:''}{d.sourceDocumentId&&<> · from {link('/sales/invoices/'+d.sourceDocumentId,'linked document')}</>}</p>
   <Statuses d={d}/>
   <p>Net {money(d.net)} · Tax {money(d.tax)} · <strong>Gross {money(d.gross)}</strong>{item&&<> · Outstanding {money(item.outstandingAmount)} (allocated {money(item.allocatedAmount)}, due {item.dueDate})</>}</p>
...
export function SalesCollections({entityId,me,can,tick,refresh}:Ctx){
 ...
 return <>{can('collection.create')&&<><h2>New receipt</h2><p>Gross equals cash plus customer withholding. Allocations are applied when the receipt posts; an unallocated remainder is a customer advance applied later.</p>
  <Editor id={'collection-new'+entityId} label="Save receipt" resetOnSave onSave={async d=>{const allocations=Object.entries(amounts).filter(([,v])=>Number(v)>0).map(([openItemId,amount])=>({openItemId,amount}));const r=await cmd.run('POST','/collections',{entityId,body:{direction:'receipt',partyId:d.partyId,currency:'PHP',valueDate:d.valueDate,grossAmount:d.grossAmount,cashAmount:d.cashAmount,withholdingAmount:d.withholdingAmount||'0',method:d.method,allocations,evidenceIds:[]}});if(r){setAmounts({});window.location.assign('/sales/collections/'+r.data.id);}return !!r;}}>
