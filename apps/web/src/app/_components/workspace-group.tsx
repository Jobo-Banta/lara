...
export function IntercompanyPairs({entityId,me,can,tick,refresh}:Ctx){
 ...
 const [targetData,setTargetData]=useState<{parties:Row[],accounts:Row[],branches:Row[],books:Row[]}|null>(null);
 const [error,setError]=useState<ApiError|null>(null);
 useEffect(()=>{let live=true;setTargetData(null);if(!target)return;Promise.all([api('GET','/parties?limit=200',{entityId:target}),api('GET','/accounts?limit=200',{entityId:target}),api('GET','/branches?limit=50',{entityId:target}),api('GET','/books?limit=20',{entityId:target})]).then(([p,a,b,k])=>{if(live)setTargetData({parties:p.data.items,accounts:a.data.items,branches:b.data.items,books:k.data.items});}).catch(e=>{if(live)setError(asError(e));});return()=>{live=false;};},[target]);
 const names=useMemo(()=>new Map((entities.items||[]).map((e:Row)=>[e.id,e.legalName])),[entities.items]);
