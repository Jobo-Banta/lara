...
const server = http.createServer((request, response) => scope.run({}, async () => {
  ...
  try {
    ...
    if(path.startsWith('/demo/')) {
      ...
      scope.getStore().run=runs.rows[0].id;
    }
    if (path.startsWith("/demo/") && config.mode !== "demo") return send(response, 404, { error: "NOT_FOUND" });
    if (path === "/demo/work") { const r = await query("select id,title,area,status,due_date,owner,source,amount::text from lara_demo.tasks order by due_date,id"); return send(response, 200, { run: scope.getStore().run, synthetic: true, tasks: r.rows }); }
    if (path === "/demo/overview") { const r = await query("select count(*)::int as tasks, coalesce(sum(amount),0)::text as amount from lara_demo.tasks"); return send(response, 200, { run: scope.getStore().run, synthetic: true, asOf: "2026-09-18", tasks: r.rows[0].tasks, amount: r.rows[0].amount }); }
